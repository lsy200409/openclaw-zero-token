...
export class QwenWebClientBrowser {
  ...
  async chatCompletions(params: {
    ...
    // Determine chat_id: use stored one from previous conversation if available
    let chatId = this.chatId || params.conversationId || null;

    // Step 1: Create a new chat session if we don't have one yet（30s 超时）
    if (!chatId) {
      ...
      this.chatId = chatId;
      console.log(`[Qwen Web Browser] Chat ID: ${chatId} (new)`);
    } else {
      console.log(`[Qwen Web Browser] Chat ID: ${chatId} (reusing)`);
    }
