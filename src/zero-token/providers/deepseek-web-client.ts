...
export class DeepSeekWebClient {
  ...
  async createChatSession(): Promise<DeepSeekChatSession> {
    ...
    const chatSession = (bizData as Record<string, unknown> | undefined)?.chat_session as
      | Record<string, unknown>
      | undefined;
    // New format: biz_data.chat_session.id
    // Old format: biz_data.id || biz_data.chat_session_id
    const sessionId =
      (typeof chatSession?.id === "string" ? chatSession.id : "") ||
      bizData?.id ||
  ...
  async chatCompletions(params: {
    ...
    // - deepseek-{pro,reasoner}*:   thinking enabled (deep thinking)
    // - deepseek-chat*:              thinking enabled by default
    // - *-nothinking:                thinking disabled
    // - *-search:                    search enabled
    const modelId = (params.model || "deepseek-chat").toLowerCase();
    const enableThinking = !modelId.includes("nothinking") && !modelId.includes("fast");
    const enableSearch = params.searchEnabled ?? modelId.includes("search");
    const isExpert = modelId.includes("pro") || modelId.includes("ultra");
    ...
    console.log(
      `[DeepSeekWebClient] Sending chat completion (session: ${params.sessionId}, model: ${modelId}, thinking: ${enableThinking}, expert: ${isExpert}, search: ${enableSearch})...`,
    );
    const res = await fetch(`https://chat.deepseek.com${targetPath}`, {
      method: "POST",
      headers: {
