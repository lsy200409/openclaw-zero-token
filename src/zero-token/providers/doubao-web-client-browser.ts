  ...
  async chatCompletions(params: {
    ...
    if (!this.conversationId && responseData.data) {
      try {
        ...
        console.log(
          `[Doubao Web Browser] Could not extract conversation_id from regex: ${String(e)}`,
        );
      }
    }

    // 方式2: 从 data: 行的 JSON 中解析，包括 event_data 内部嵌套的 JSON
    if (!this.conversationId && responseData.data) {
      try {
        const lines = responseData.data.split("\n");
        for (const line of lines) {
          if (line.startsWith("data:")) {
            ...
            // 尝试顶层字段
            const topId = parsed.conversation_id || parsed.data?.conversation_id;
            if (topId && topId !== "0") {
              this.conversationId = String(topId);
              console.log(`[Doubao Web Browser] Captured conversation_id: ${this.conversationId}`);
              break;
            }
            // 尝试 event_data 内部（JSON 编码字符串）
            if (typeof parsed.event_data === "string") {
              try {
                const inner = JSON.parse(parsed.event_data);
                const innerId = inner.conversation_id || inner.data?.conversation_id;
                if (innerId && innerId !== "0") {
                  this.conversationId = String(innerId);
                  console.log(
                    `[Doubao Web Browser] Captured conversation_id from event_data: ${this.conversationId}`,
                  );
                  break;
                }
              } catch {
