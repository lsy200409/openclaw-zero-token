  ...
  return (model, context, streamOptions) => {
    ...
    const run = async () => {
      try {
        ...
        // Restore previous session's ID on the client
        if (sessionId) {
        ...
        const processLine = (line: string) => {
          ...
          try {
            ...
            // Qwen CN Web returns sessionid in communication field
            if (data.communication?.sessionid) {
              sessionMap.set(providerKey, data.communication.sessionid);
            }
        ...
        // Sync client's sessionId back to session map
        const clientSessionId = client.getConversationId();
        if (clientSessionId) {
          sessionMap.set(providerKey, clientSessionId);
        }
