...
import {
  loadDeepseekSessions,
  saveDeepseekSessionEntry,
  saveDeepseekParentMessageId,
  removeDeepseekSessionEntry,
} from "./deepseek-session-store.js";
...
export function createDeepseekWebStreamFn(cookieOrJson: string): StreamFn {
  ...
  const persisted = loadDeepseekSessions();
  const sessionMap = new Map<string, string>(Object.entries(persisted.sessionMap));
  const parentMessageMap = new Map<string, string | number>(
    Object.entries(persisted.parentMessageMap),
  );
  ...
  return (model, context, options) => {
    ...
    const run = async () => {
      ...
      try {
        ...
        sessionKey = (context as unknown as { sessionId?: string }).sessionId || "default";
        ...
        if (!dsSessionId) {
          ...
          saveDeepseekSessionEntry(sessionKey, dsSessionId, null);
          parentId = undefined;
        }
        ...
        // Retry loop: on biz_code errors (e.g. parent message ID stale),
        // reset parentId and re-request so caller gets a valid response.
        let _processingDone = false;
        let _error26Detected = false;

        while (!_processingDone) {
          _error26Detected = false;
              ...
              try {
                ...
                if (data.response_message_id) {
                  if (data.response_message_id !== parentMessageMap.get(sessionKey)) {
                    ...
                    saveDeepseekParentMessageId(sessionKey, data.response_message_id);
                  }
                }
          ...
          while (true) {
            ...
            if (done) {
              ...
              // JSON fallback: DeepSeek may return application/json (expert mode error)
              if (totalProcessed === 0 && rawChunks.length > 0) {
                const rawText = rawChunks.join("");
                const json = tryParseJson(rawText);
                if (json) {
                  const content = extractJsonContent(json);
                  if (content) {
                    if (content.startsWith("[DeepSeek error")) {
                      console.warn(
                        `[DeepseekWebStream] ${content} — resetting session state for ${sessionKey}`,
                      );
                      parentMessageMap.delete(sessionKey);
                      sessionMap.delete(sessionKey);
                      removeDeepseekSessionEntry(sessionKey);
                      dsSessionId = "";
                      _error26Detected = true;
                    } else {
                      pushDelta(content);
                    }
                  }
          ...
          if (_error26Detected) {
            // Clear stale session state and create a new one
            parentId = undefined;
            if (!dsSessionId) {
              ...
              sessionMap.set(sessionKey, dsSessionId);
              saveDeepseekSessionEntry(sessionKey, dsSessionId, null);
              console.log(`[DeepseekWebStream] Created new DS session after error: ${dsSessionId}`);
            }
            // Rebuild prompt from full history for retry
            prompt = "";
            const historyParts: string[] = [];
            const tools = context.tools || [];
            let systemPromptContent = systemPrompt;
                ...
                if (msg.role === "toolResult") {
                  ...
                  const resultText = Array.isArray(msg.content)
                    ? (msg.content as Array<{ type: string; text?: string }>)
                        ...
                        .join("")
                    : msg.content;
                  historyParts.push(`[Tool ${toolName} returned]: ${resultText}`);
                  ...
                  if (Array.isArray(msg.content)) {
                    for (const part of msg.content as Array<{
                      ...
                      if (part.type === "text") {
                        ...
                        assistantContent += `<tool_call id="${part.id ?? ""}" name="${part.name ?? ""}">${JSON.stringify(part.arguments)}</tool_call>`;
                      }
                    }
                  } else {
                  ...
                  const text = Array.isArray(msg.content)
                    ...
                    : msg.content;
                  if (text) {
                    historyParts.push(`${msg.role}: ${text}`);
                  }
                }
              }
            }
            prompt = historyParts.join("\n\n");
            continue; // re-enter outer while loop (retry)
          }
        ...
        console.warn(`[DeepseekWebStream] Error occurred: ${errorMessage}`);
...
function extractJsonContent(json: Record<string, unknown>): string | null {
  ...
  if (typeof json?.code === "number") {
    ...
    if (typeof data?.biz_code === "number" && data.biz_code !== 0) {
      return `[DeepSeek error ${data.biz_code}: ${data.biz_msg || "unknown"}]`;
    }
  }
  return null;
}

function extractJsonReasoning(json: Record<string, unknown>): string | null {
  const choices = json?.choices as Array<{ message?: { reasoning_content?: string } }> | undefined;
  if (choices?.[0]?.message?.reasoning_content) return choices[0].message.reasoning_content;
  if (typeof json?.reasoning_content === "string") return json.reasoning_content;
  if (typeof json?.reasoning === "string") return json.reasoning;
  return null;
}
