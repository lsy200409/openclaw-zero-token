...
export function installToolResultContextGuard(params: {
  ...
  mutableAgent.transformContext = (async (messages: AgentMessage[], signal: AbortSignal) => {
    ...
    //
    // NOTE: We intentionally do NOT throw here. Throwing from within transformContext
    // causes an unhandled promise rejection inside pi-agent-core's runLoop/streamAssistantResponse,
    // which crashes the stream and disconnects the WebSocket session.
    // Instead, we warn and let the messages through — if the context is truly too large,
    // the LLM API itself will return a proper error that IS caught by the agent loop's
    ...
    if (postEnforcementChars > preemptiveOverflowChars) {
      console.warn(
        `[tool-result-context-guard] Context size (${postEnforcementChars} chars) exceeds ` +
        `preemptive threshold (${preemptiveOverflowChars} chars). ` +
        `Tool results have been compacted as much as possible. ` +
        `Letting request proceed; LLM API will reject if truly over limit.`,
      );
    }
