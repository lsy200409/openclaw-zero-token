...
export class GatewayClient {
  ...
  private maxTickMissesBeforeWarn = 2;
  ...
  private scheduleReconnect() {
    ...
    if (this.maxReconnectAttempts > 0 && this.reconnectAttempts > this.maxReconnectAttempts) {
      logError(
        `gateway reconnect attempts exceeded limit (${this.maxReconnectAttempts}), giving up`,
      );
      this.flushPendingErrors(new Error("gateway reconnect limit exceeded"));
      return;
    }
    ...
    logDebug(
      `gateway scheduling reconnect attempt ${this.reconnectAttempts} in ${Math.round(actualDelay)}ms`,
    );
    setTimeout(() => this.start(), actualDelay).unref();
  }
  ...
  private startTickWatch() {
    ...
    this.tickTimer = setInterval(() => {
      ...
      if (gap > this.tickIntervalMs * 3) {
        ...
        logDebug(
          `gateway tick timeout detected (gap=${gap}ms, misses=${this.consecutiveTickMisses})`,
        );
